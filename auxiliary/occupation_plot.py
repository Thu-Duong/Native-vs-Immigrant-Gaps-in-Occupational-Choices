...
def plot_occupations_by_year(df):
    ...
    --------
        Figure.
    """
    labels = ['professionals','technicians','white_collar_worker','blue_collar_worker','elementary','military']
    #'training_schooling','unemployed','maternity_leave'

    fig, ax = plt.subplots(figsize=(12,8))

    shares = df.groupby("syear").isco88_choices.value_counts(normalize=True).unstack()[labels] * 100
...
def plot_occupations_by_age(df):
    ...
    shares = df.groupby("bioage").isco88_choices.value_counts(normalize=True).unstack()[labels] * 100
    shares.plot.bar(stacked=True, ax=ax, width=0.8, cmap='RdBu')
...
def plot_occupations_by_yearbirth(df):
    ...
    # Choices should be ordered: 'professionals','technicians','white_collar_worker','blue_collar_worker','elementary','military'
    shares = df.groupby('gebjahr').isco88_choices.value_counts(normalize=True).unstack()[labels] * 100
    shares.plot.bar(stacked=True, ax=ax, width=0.8, cmap='RdBu')
...
def plot_country_origin_by_year(df,labels):
...
def plot_corr_occupation_by_country_origin(df):
    
    heatmap = pd.crosstab(df.country_origin, df.isco88_choices, normalize='index')
...
def plot_corr_occupation_by_fcountry_origin(df):
    ...
    heatmap = pd.crosstab(df.forigin_group, df.isco88_choices, normalize='index')
